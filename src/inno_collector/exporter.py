...
def _object_list(payload: dict, field: str) -> list[dict]:
    ...
    return validate_object_rows(rows, field)


def validate_object_rows(rows: object, field: str) -> list[dict]:
    if not isinstance(rows, list) or any(not isinstance(row, dict) for row in rows):
        raise ExporterCommandError(f"exporter returned invalid {field}")
    return rows


def validate_success_payload(payload: object) -> dict:
    if not isinstance(payload, dict):
        raise ExporterCommandError("exporter returned invalid JSON object")
    if payload.get("ok") is not True:
        message = payload.get("error") or "exporter command failed"
...
def validate_sync_payload(
    payload: object,
    account_id: int,
    error_fallback: str = "exporter command failed",
) -> dict:
    if not isinstance(payload, dict):
        raise ExporterCommandError("exporter returned invalid JSON object")
    ...
    if (
        ...
        return {**payload, "errors": [_sanitize(item) for item in errors]}

    partial_fields = {"account_id", "fetched_count", "upserted_count", "errors"}
    if payload.get("ok") is False and partial_fields.intersection(payload):
        raise ExporterCommandError("exporter returned invalid partial sync response")
    message = payload.get("error") or error_fallback
    raise ExporterCommandError(_sanitize(str(message)))


def validate_download_payload(payload: object) -> dict:
    if not isinstance(payload, dict):
        raise ExporterCommandError("exporter returned invalid JSON object")
    if payload.get("ok") is True:
        return payload
    ...
    count_fields = ("selected_count", "success_count", "failure_count", "skipped_count")
    ...
    if (
        ...
        raise ExporterCommandError("exporter returned invalid partial download response")
    return payload
...
class MooreExporterAdapter:
    ...
    def _run_sync(self, account_id: int, *arguments: str) -> dict:
        ...
        if code in {0, 1}:
            return validate_sync_payload(
                payload,
                account_id,
                stderr or "exporter command failed",
            )
        message = payload.get("error") or stderr or "exporter command failed"
        raise ExporterCommandError(_sanitize(str(message)))
